...
const TW_HOLIDAYS = {
  "2026-01-01":{n:"元旦",t:"假"},
  "2026-02-14":{n:"春節連假",t:"假"},"2026-02-16":{n:"除夕",t:"假"},
  "2026-02-17":{n:"初一",t:"假"},"2026-02-18":{n:"初二",t:"假"},
  "2026-02-19":{n:"初三",t:"假"},"2026-02-20":{n:"春節補假",t:"補"},
  "2026-02-27":{n:"228連假",t:"假"},"2026-02-28":{n:"228紀念日",t:"假"},
  "2026-03-01":{n:"228補假",t:"補"},
  "2026-04-03":{n:"兒童節補假",t:"補"},"2026-04-04":{n:"兒童節",t:"假"},
  "2026-04-05":{n:"清明節",t:"假"},"2026-04-06":{n:"清明補假",t:"補"},
  "2026-05-01":{n:"勞動節",t:"假"},"2026-05-02":{n:"勞動節連假",t:"補"},
  "2026-05-03":{n:"勞動節連假",t:"補"},
  "2026-06-19":{n:"端午節",t:"假"},"2026-06-20":{n:"端午連假",t:"補"},
  "2026-09-25":{n:"中秋節",t:"假"},"2026-09-26":{n:"中秋連假",t:"補"},
  "2026-09-28":{n:"教師節",t:"假"},
  "2026-10-09":{n:"國慶連假",t:"補"},"2026-10-10":{n:"國慶日",t:"假"},
  "2026-10-11":{n:"國慶連假",t:"補"},
  "2026-12-25":{n:"行憲紀念日",t:"假"},"2026-12-26":{n:"行憲連假",t:"補"},
};
...
const VISIT_FORMS = {
  "安親寄宿":[
    {title:"🐾 基本健康資訊",fields:[
      ...
      {key:"mealHabit",label:"用餐習慣？（飼料？一天幾餐？）"},
...
const VISIT_GUIDES = {
  "安親寄宿":{
    ...
    steps:[
      {n:"01",t:"認識毛爸媽",c:"#9B7553",s:"「OO媽媽您好，我是保姆XXX，今天主要是來多認識毛孩，確認平常的照顧方式和確認細節，也會照您們習慣試著相處看看」"},
      {n:"02",t:"認識毛小孩",c:"#C19A6B",s:"「請問OO媽媽，他會怕陌生人或陌生環境嗎？有食物過敏嗎？我有些小零食，可以給他互相認識一下！」"},
      {n:"03",t:"邀請填寫家訪表",c:"#7A9E7E",s:"「這邊是關於照顧OO的一些問題，包含緊急聯絡資訊、常用動物醫院、習性、健康狀況等等，麻煩您幫我填一下喔」"},
      {n:"04",t:"確認細節・安全",c:"#C8845A",s:"「填完了嗎？我們來確認一下您剛剛填的內容喔」\n\n提醒：服務含國泰保險保障、全程照片回報，確認是否需要Google Maps紀錄路線。"},
      {n:"05",t:"試安親 30分鐘～6小時",c:"#B07878",s:"在飼主不在的情況下簡單試安親，了解毛孩適應情況。\n\n⚠️ 若您或飼主覺得不合適，可取消訂單。"},
    ],
  ...
  "到府照顧":{
    ...
    steps:[
      {n:"01",t:"認識毛爸媽",c:"#9B7553",s:"「OO媽媽您好，我是保姆XXX，今天主要是來多認識毛孩，確認你們平常的照顧方式和確認細節。」"},
      {n:"02",t:"認識毛小孩",c:"#C19A6B",s:"「請問OO媽媽，他會怕陌生人嗎？有食物過敏嗎？我有帶些小零食，可以給他互相認識一下！」\n\n💡 膽小毛孩請仔細觀察，不要強硬靠近。"},
      {n:"03",t:"邀請填寫家訪表",c:"#7A9E7E",s:"「這邊是關於照顧OO的一些問題，麻煩您幫我填一下喔」"},
      {n:"04",t:"確認細節・安全・付款",c:"#C8845A",s:"確認：服務含國泰保險保障、全程照片回報，確認攝影機需求與鑰匙交付，並提醒完成付款。"},
    ],
  },
  "陪伴散步":{
    ...
    steps:[
      {n:"01",t:"認識毛爸媽",c:"#9B7553",s:"「OO媽媽您好，我是保姆XXX，今天主要是來多認識毛孩，也會照您們習慣試遛他一次」"},
      {n:"02",t:"認識毛小孩",c:"#C19A6B",s:"「請問OO媽媽，他會怕陌生人嗎？我有帶些小零食，可以給他互相認識！」\n\n💡 膽小毛孩時刻仔細觀察，不用強硬靠近。"},
      {n:"03",t:"邀請填寫家訪表",c:"#7A9E7E",s:"「這邊是關於照顧OO的一些問題，麻煩您幫我填一下喔」"},
      {n:"04",t:"確認細節・安全・付款",c:"#C8845A",s:"確認：服務含國泰保險保障、全程照片回報，確認Google Maps路線、攝影機需求、鑰匙交付，並提醒完成付款。"},
      {n:"05",t:"試遛狗五分鐘",c:"#B07878",s:"試穿牽繩和背套，在飼主不在的情況下簡單試遛五分鐘。"},
    ],
  },
  "到府美容/洗澡":{
    ...
    steps:[
      {n:"01",t:"事前確認",c:"#9B7553",s:"去執行服務前一定要跟飼主確認毛孩個性與洗澡細節（是否有攻擊性、皮膚過敏、是否自備洗毛精等）。"},
      {n:"02",t:"建立信任",c:"#C19A6B",s:"先與毛孩玩耍、給零食培養感情，讓毛孩不會太緊張，同時蒐集美容前的對比照！"},
      {n:"03",t:"執行美容",c:"#7A9E7E",s:"洗澡時需幫毛孩清耳朵、清眼屎、擠肛門腺。可依需求提供剪指甲、修腳底毛、修屁股毛等附加服務。\n\n⚠️ 若毛孩太兇無法執行，可取消訂單，平台會全額退費。"},
      {n:"04",t:"注意事項",c:"#C8845A",s:"發現皮膚異常需回報飼主。剪指甲流血可使用止血粉；若指甲過長，要先提醒飼主可能流血。"},
      {n:"05",t:"完成收尾",c:"#B07878",s:"美容完給毛孩零食獎勵！蒐集美容後的對比照。服務結束清理地上毛髮，清理、消毒自己的工具。"},
    ],
  },
};
...
const cardSt = {background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:20,boxShadow:"0 2px 12px rgba(154,117,83,.08)"};
const inp = {width:"100%",boxSizing:"border-box",background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,color:C.text,padding:"9px 13px",fontSize:14,outline:"none",fontFamily:"inherit"};
const lbl = {fontSize:12,color:C.muted,marginBottom:5,display:"block",letterSpacing:".4px"};
const btnP = {fontFamily:"inherit",cursor:"pointer",background:`linear-gradient(135deg,${C.accent},${C.accent2})`,color:"#fff",border:"none",borderRadius:12,padding:"10px 18px",fontWeight:600,fontSize:14};
const btnG = {fontFamily:"inherit",cursor:"pointer",background:C.surface,color:C.muted,border:`1px solid ${C.border}`,borderRadius:12,padding:"10px 18px",fontWeight:500,fontSize:14};
const btnD = {fontFamily:"inherit",cursor:"pointer",background:"#FDF0EE",color:C.red,border:`1px solid ${C.red}44`,borderRadius:12,padding:"10px 18px",fontWeight:500,fontSize:14};
...
function Toast({message,show,error}){
  if(!show) return null;
  return <div style={{position:"fixed",bottom:28,left:"50%",transform:"translateX(-50%)",zIndex:3000,background:error?C.red:C.green,color:"#fff",borderRadius:12,padding:"10px 22px",fontSize:14,fontWeight:600,boxShadow:"0 4px 20px rgba(0,0,0,.2)",fontFamily:"inherit",whiteSpace:"nowrap"}}>{error?"❌":"✅"} {message}</div>;
}
function ConfirmDialog({open,message,onConfirm,onCancel}){
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(74,55,40,.5)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:C.card,borderRadius:20,padding:28,width:"100%",maxWidth:360,border:`1px solid ${C.border}`,textAlign:"center"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {f.pets.map((pet,i)=>(
              <div key={i} style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                <select style={{...inp,flex:1,height:40,padding:"0 10px"}} value={pet.type} onChange={e=>setPet(i,"type",e.target.value)}>
...
function Calendar({bookings,onSelectDate,selectedDate}){
  ...
  const moHols=Object.entries(TW_HOLIDAYS).filter(([d])=>d.startsWith(`${yr}-${String(mo+1).padStart(2,"0")}`));
  return (
    <div style={cardSt}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <div style={{textAlign:"center"}}>
          ...
          {moHols.length>0&&<div style={{fontSize:11,color:C.dim,marginTop:1}}>🎉 本月有 {moHols.length} 個假日/補休</div>}
        </div>
        <button onClick={()=>setView(new Date(yr,mo+1,1))} style={{...btnG,padding:"6px 14px",fontSize:18}}>›</button>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
        {cells.map((d,i)=>{
          ...
          return (
            <div key={i} onClick={()=>onSelectDate(dateStr)} style={{minHeight:60,borderRadius:10,padding:"4px 5px",cursor:"pointer",background:isSel?`${C.accent}18`:isHol?C.holidayBg:isMakeup?C.makeupBg:isToday?`${C.accent2}10`:"transparent",border:isSel?`1.5px solid ${C.accent}`:isToday?`1px solid ${C.accent2}66`:`1px solid transparent`,transition:"all .13s"}}>
              <div style={{fontSize:12,fontWeight:isToday||isSel?700:400,lineHeight:1,marginBottom:1,color:isHol?C.holiday:isMakeup?C.makeup:isSel?C.accent:isToday?C.accent:dow===0?C.holiday:dow===6?C.orange:C.text}}>{d}</div>
              {h&&<div style={{fontSize:7.5,lineHeight:1.2,marginBottom:2,color:isHol?C.holiday:C.makeup,fontWeight:600}}>{h.n.slice(0,5)}</div>}
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:10,marginTop:12,paddingTop:10,borderTop:`1px solid ${C.border}`}}>
        <div style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:C.muted}}><div style={{width:10,height:10,borderRadius:2,background:C.holidayBg,border:`1px solid ${C.holiday}55`}}/>國定假日</div>
        <div style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:C.muted}}><div style={{width:10,height:10,borderRadius:2,background:C.makeupBg,border:`1px solid ${C.makeup}55`}}/>補休</div>
        {Object.entries(STATUS_MAP).map(([k,v])=>(
          <div key={k} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:C.muted}}><div style={{width:8,height:8,borderRadius:"50%",background:v.color}}/>{v.label}</div>
        ))}
      </div>
    </div>
  );
...
// ══════════════════════════════════════
// VisitForm（修正：依服務類型顯示不同問題 + 圖片上傳）
// ══════════════════════════════════════
function VisitForm({booking,savedData,onSave,onClose,loading}){
  const [form,setForm]=useState(savedData||{});
  const [editing,setEditing]=useState(!savedData||Object.keys(savedData).filter(k=>!["id","bookingId","title","飼主姓名","飼主電話","寵物名稱","寵物種類","服務類型"].includes(k)).length===0);
  const [images,setImages]=useState(savedData?.imageUrls?savedData.imageUrls.split(',').filter(Boolean):[]);
    ...
    files.forEach(file=>{
      ...
      reader.onload=ev=>{
        setImages(prev=>{
          ...
          setForm(f=>({...f,imageUrls:next.join(',')}));
          return next;
        });
      };
  ...
  return (
    <div>
      ...
      {!editing&&<div style={{background:"#F0F7F2",border:"1px solid #BFD9C8",borderRadius:10,padding:"9px 14px",marginBottom:16,fontSize:13,color:C.green}}>👁️ 查看模式 — 點「編輯」可修改內容</div>}
      {sections.map(sec=>(
        <div key={sec.title} style={{marginBottom:18}}>
          <div style={{fontSize:13,fontWeight:700,color:C.accent,marginBottom:8,paddingBottom:6,borderBottom:`1px solid ${C.border}`}}>{sec.title}</div>
      ...
      <div style={{marginBottom:18}}>
        <div style={{fontSize:13,fontWeight:700,color:C.accent,marginBottom:8,paddingBottom:6,borderBottom:`1px solid ${C.border}`}}>📷 家訪照片</div>
        {images.length>0&&(
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:10}}>
            {images.map((img,i)=>(
              <div key={i} style={{position:"relative",borderRadius:10,overflow:"hidden",aspectRatio:"1",background:C.surface}}>
                ...
                {editing&&(
                  <button onClick={()=>removeImage(i)} style={{position:"absolute",top:4,right:4,background:"rgba(0,0,0,.55)",color:"#fff",border:"none",borderRadius:"50%",width:22,height:22,cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit"}}>✕</button>
                )}
              </div>
            ))}
          </div>
        ...
        {editing&&(
          <>
            ...
            <button onClick={()=>fileRef.current.click()} style={{...btnG,fontSize:13,padding:"8px 16px"}}>📷 上傳照片</button>
          </>
        )}
        {!editing&&images.length===0&&<div style={{color:C.dim,fontSize:13}}>尚未上傳照片</div>}
      </div>
...
function ServiceGuide({onClose}){
  ...
  const tabBtn=(id,label)=>(
    <button onClick={()=>setTab(id)} style={{fontFamily:"inherit",cursor:"pointer",background:tab===id?`linear-gradient(135deg,${C.accent},${C.accent2})`:C.surface,color:tab===id?"#fff":C.muted,border:`1px solid ${tab===id?C.accent:C.border}`,borderRadius:10,padding:"8px 16px",fontWeight:600,fontSize:13}}>{label}</button>
  );
    ...
    <div>
      ...
      {(tab==="visit"||tab==="service")&&(
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:18}}>
          {SERVICE_TYPES.map(t=>(
            <button key={t} onClick={()=>setSvcType(t)} style={{fontFamily:"inherit",cursor:"pointer",background:svcType===t?C.soft:C.surface,color:svcType===t?C.accent:C.muted,border:`1px solid ${svcType===t?C.accent2:C.border}`,borderRadius:20,padding:"5px 14px",fontSize:12.5,fontWeight:svcType===t?700:400}}>{SERVICE_ICONS[t]} {t}</button>
          ))}
        </div>
      )}
      {tab==="visit"&&(
        <div>
          <div style={{background:"#FEF6F0",border:"1px solid #E8C4A8",borderRadius:12,padding:"13px 15px",marginBottom:18,fontSize:13.5,color:C.muted,lineHeight:1.7}}>
          ...
          {guide.prep.map((p,i)=><div key={i} style={{background:C.surface,borderRadius:10,padding:"10px 14px",marginBottom:8,fontSize:13,color:C.muted,lineHeight:1.6}}>{p}</div>)}
          <div style={{fontSize:13,fontWeight:700,color:C.accent,margin:"18px 0 10px"}}>📍 家訪步驟</div>
          {guide.steps.map(vs=>(
            <div key={vs.n} style={{borderRadius:12,padding:"13px 15px",marginBottom:10,background:C.card2,border:`1px solid ${C.border}`,borderLeft:`3px solid ${vs.c}`}}>
          ...
          {guide.extra&&<div style={{background:"#F0F7F2",border:"1px solid #BFD9C8",borderRadius:12,padding:"12px 14px",marginTop:8,fontSize:13,color:C.muted,lineHeight:1.7}}>{guide.extra}</div>}
        </div>
      )}
      {tab==="service"&&(
        <div>
      ...
      {tab==="course"&&(
        <div>
          <div style={{background:"#F0F7F2",border:"1px solid #BFD9C8",borderRadius:12,padding:"14px 16px",marginBottom:18}}>
            ...
            <div style={{fontSize:13,color:C.muted,lineHeight:1.7}}>完全免費！只需註冊台北通或e大會員帳號，即可開始學習。</div>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
            {tags.map(t=><button key={t} onClick={()=>setCourseTag(t)} style={{fontFamily:"inherit",cursor:"pointer",background:courseTag===t?C.accent:C.surface,color:courseTag===t?"#fff":C.muted,border:`1px solid ${courseTag===t?C.accent:C.border}`,borderRadius:20,padding:"5px 14px",fontSize:12.5}}>{t}</button>)}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>
            {filteredCourses.map((c,i)=>(
              <a key={i} href={c.url} target="_blank" rel="noreferrer" style={{textDecoration:"none",display:"block",...cardSt,padding:"14px 16px"}}>
        ...
        <div style={{maxWidth:680,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              {loading&&<div style={{fontSize:12,color:C.dim}}>⏳ 載入中</div>}
          ...
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            {[{label:"總預約",val:bookings.length,c:C.accent},{label:"今日服務",val:bookings.filter(b=>(b.dates||[]).includes(today)).length,c:C.green},{label:"待確認",val:bookings.filter(b=>b.status==="pending").length,c:C.orange},{label:"完成收入",val:`$${completedIncome.toLocaleString()}`,c:C.green}].map(s=>(
              <div key={s.label} style={{flex:1,textAlign:"center",background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"10px 4px"}}>
          ...
          {upcomingHolidays.length>0&&(
            <div style={{display:"flex",gap:8,marginBottom:12,overflowX:"auto",paddingBottom:2}}>
              {upcomingHolidays.map(([d,h])=>(
                <div key={d} style={{display:"flex",alignItems:"center",gap:5,background:h.t==="補"?C.makeupBg:C.holidayBg,border:`1px solid ${h.t==="補"?C.makeup+"33":C.holiday+"33"}`,borderRadius:20,padding:"4px 12px",whiteSpace:"nowrap",fontSize:11.5,color:h.t==="補"?C.makeup:C.holiday}}>
          ...
          <div>
            ...
            {selectedDate&&(
              <div style={{marginTop:14,...cardSt}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  <div>
                    <span style={{fontWeight:700,color:C.text,fontSize:15}}>{selectedDate.slice(5).replace("-","/")} 的預約</span>
                    <span style={{...pill(C.accent),marginLeft:8}}>{dayBookings.length} 筆</span>
                    {TW_HOLIDAYS[selectedDate]&&<div style={{fontSize:12,color:TW_HOLIDAYS[selectedDate].t==="補"?C.makeup:C.holiday,marginTop:2}}>{TW_HOLIDAYS[selectedDate].t==="補"?"🔄":"🎉"} {TW_HOLIDAYS[selectedDate].n}</div>}
                  </div>
